...
export default function ProductDetails() {
  ...
  // NEW: Buy Now handler — directly open checkout page with this single product
  const handleBuyNow = () => {
    if (!p) {
    ...
    // Build a minimal item shape that BuyNow expects
    const singleItem = {
      id: p._id || id,
      _id: p._id || id,
      title: title,
    ...
    // Navigate to BuyNow page and pass items in location.state
    // quick:true can be used by BuyNow to auto-select payment/flow if you support it
    navigate("/buy-now", { state: { items: [singleItem], quick: true } });
  };
